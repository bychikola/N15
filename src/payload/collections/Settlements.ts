 ...
 * кода сайта.
 *
 * «Показывать всегда»: галочка — строка видна на странице «Межрегиональная
 * недвижимость» (/interregional) постоянно (даже без объектов, с пометкой
 * «Объектов Н15 пока нет»); без галочки населённый пункт появляется там, когда
 * в нём есть опубликованные объекты — так длинный список «и другие населённые
 * пункты» не выдумывает предложения. Фильтр «Город» каталога показывает весь
 * справочник региона независимо от галочки (см. loadCatalogCityFilter).
 */
