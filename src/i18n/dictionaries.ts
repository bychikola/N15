...
export const ru = {
  ...
  crm: {
    ...
    agentSettingsHint: 'Конфигурация живёт на сервере: ~/.claude/settings.json или .env воркера (tools/agent-worker). Провайдер — DeepSeek (ANTHROPIC_BASE_URL=https://api.deepseek.com/anthropic, модель deepseek-v4-flash). Ключ и права задаются там же — из CRM настройки не меняются.',
    agentConfigHint: 'Переменные окружения для Claude Code CLI (JSON). Воркер применяет их перед каждым запуском. Например: ANTHROPIC_BASE_URL, ANTHROPIC_AUTH_TOKEN, ANTHROPIC_MODEL.',
...
const os: Dict = {
  ...
  crm: {
    ...
    agentSettingsHint: 'Конфигураци серверыл цæры: ~/.claude/settings.json æви .env воркеры (tools/agent-worker). Провайдер — DeepSeek (ANTHROPIC_BASE_URL=https://api.deepseek.com/anthropic, модель deepseek-v4-flash). Ключ æмæ бартæ дзы æвæрд цæуынц — CRM-æй конфигураци нæ ивы.',
    agentConfigHint: 'Claude Code CLI-йы æмбæлгæттæ (JSON). Воркер сæ пайда кæны алы ранæгуылæй. Зæгъæм: ANTHROPIC_BASE_URL, ANTHROPIC_AUTH_TOKEN, ANTHROPIC_MODEL.',
