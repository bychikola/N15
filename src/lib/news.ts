...
export const NEWS_TOPICS: { slug: NewsTopic; label: string; stems: string[] }[] = [
  {
    slug: 'mortgage',
    label: 'Ипотека',
    stems: ['ипотек', 'жилищн.{0,12}кредит', 'семейн.{0,12}ипотек', 'ипотечн'],
  },
  {
    slug: 'rates',
    label: 'Ставки и банковские программы',
    stems: ['ставк', 'ключев', 'процентн', 'банковск', 'рефинанс', 'льготн', 'вклад', 'кредит'],
  },
  {
    slug: 'registration',
  ...
  },
  {
    ...
    stems: ['сделк', 'купл', 'продаж', 'задат', 'эскроу', 'дду', 'долев.{0,12}участ', 'аукцион'],
  },
  {
    slug: 'legislation',
 ...
 */
const REALTY =
  /ипотек|недвижим|жилищн|жиль|квартир|застройщик|новостройк|долев|кадастр|росреестр|егрн|земл|сделк|регистрац|многоквартир|апартамент|градостроит|реновац|эскроу|дду|ижс|снт|аренд|строит|имущественн.{0,15}налог|налог.{0,15}(недвижим|земл|имуществ|вычет)/
const FINANCE =
  /ключев|процентн.{0,12}ставк|ставк.{0,12}(ипотек|кредит|вклад|банк|жилищ)|банковск.{0,15}(программ|услуг|ставк)|рефинанс|льготн.{0,15}(ипотек|кредит)|вклад|кредитн.{0,15}(лини|программ)/
const RELEVANCE = new RegExp(`${REALTY.source}|${FINANCE.source}`)

export const isRelevantNews = (...parts: (string | null | undefined)[]): boolean =>
  RELEVANCE.test(normalizeNewsText(parts))
 ...
 */
export function classifyNewsTopic(...parts: (string | null | undefined)[]): NewsTopic | null {
  const text = normalizeNewsText(parts)
  if (!text.trim() || !RELEVANCE.test(text)) return null
...
/** Сколько последних записей берём из каждого канала за проход */
export const NEWS_ITEMS_PER_FEED = 12
