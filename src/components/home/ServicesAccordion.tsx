...
// Две компактные карточки-гармошки главной страницы: «Дизайн и ремонт под
// ключ» и «Юридические услуги» стоят рядом (на телефоне — друг под другом).
// В закрытом виде видна только строка заголовка со стрелкой, перечень услуг
// раскрывается нажатием на заголовок: карточка — нативный <details>, поэтому
// раскрытие работает без клиентского JS (как у меню «Межрегиональная
// недвижимость» в шапке).
//
...
// Карточка-гармошка: заголовок со стрелкой и перечень услуг во внутренних
// строках; cta — необязательная последняя строка-призыв (консультация),
  ...
  return (
    <details className="lp-services-card" id={id}>
      <summary className="lp-services-head">
        ...
        <i className="lp-services-arrow" aria-hidden="true" />
      </summary>
