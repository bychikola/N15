    ...
    <>
      ...
      <main className="pt-20">
        <SectionWrapper variant="dark" ornament="floral">
          <h1 className="text-4xl md:text-5xl font-[family-name:var(--font-display)] text-[var(--n15-white)] mb-4">{t.blog.title}</h1>
          <p className="text-[var(--n15-muted)] max-w-xl">{t.blog.subtitle}</p>
        </SectionWrapper>

        <SectionWrapper variant="charcoal">
          {postsList.length === 0 ? (
            <p className="text-center py-20 text-[var(--n15-muted)]">{t.blog.empty}</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {postsList.map((post) => (
                <a key={post.id} href={`/${lang}/blog/${post.id}`} className="group border border-[var(--n15-gold)]/10 hover:border-[var(--n15-gold)]/30 transition-all duration-300">
                  ...
                  <div className="p-6">
                    ...
                    <span className="text-xs text-[var(--n15-muted)]">{post.publishedAt ? new Date(post.publishedAt).toLocaleDateString(t.locale) : ''}</span>
                  </div>
                </a>
              ))}
            </div>
