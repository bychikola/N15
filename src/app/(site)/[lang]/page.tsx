      ...
      <main>
                ...
                <OrnamentBorder cornerOrnament={obj.isPremium}>
                  <div className="p-6 group">
                    ...
                    <p className="text-xs text-[var(--n15-muted)] mb-3">
                      {[obj.address?.street, obj.address?.house].filter(Boolean).join(', ') || t.home.defaultAddress}
                    </p>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-[var(--n15-muted)]">
                        {obj.area && `${obj.area} ${t.catalog.sqm}`}{obj.rooms && ` • ${obj.rooms} ${t.catalog.rooms}`}
                      </span>
                    </div>
                  </div>
                </OrnamentBorder>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl md:text-4xl font-[family-name:var(--font-display)] text-[var(--n15-white)] mb-6">
                {t.home.aboutTitle1}{' '}
                <span className="text-[var(--n15-gold)]">{t.home.aboutTitle2}</span>
                {' '}{t.home.aboutTitle3}
              </h2>
              ...
              <div className="grid grid-cols-3 gap-8 mt-10">
                {[
                  { value: `${yearsActive}+`, label: t.home.statYears },
                  { value: `${Math.max(totalDeals, 850)}+`, label: t.home.statDeals },
                  { value: `${agents.length}`, label: t.home.statExperts },
                ].map((s) => (
                  <div key={s.label}>
              ...
              <Button variant="primary" href={`/${lang}/about`} className="mt-8">
                {t.home.learnMore}
              </Button>
            </div>
        ...
        <SectionWrapper variant="charcoal">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {posts.map((post) => (
              <a key={post.id} href={`/${lang}/blog/${post.id}`} className="group p-6 border border-[var(--n15-gold)]/10 hover:border-[var(--n15-gold)]/30 transition-all duration-300">
                <span className="text-[10px] tracking-[0.2em] uppercase text-[var(--n15-gold)]/60">
                  {post.category || t.home.articleFallback}
                </span>
                <h3 className="text-base text-[var(--n15-white)] mt-2 mb-3 group-hover:text-[var(--n15-gold)] transition-colors">
                  {post.title}
                </h3>
                <span className="text-xs text-[var(--n15-muted)]">
                  {post.publishedAt ? new Date(post.publishedAt).toLocaleDateString(t.locale) : ''}
                </span>
              </a>
            ))}
          </div>
